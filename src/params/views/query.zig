...
test "iterator: yields every entity across all matching archetypes" {
    ...
    _ = world.addOwnedEntity(allocator, .{ Position{ .x = 2, .y = 2 }, Velocity{ .dx = 2, .dy = 2 } });
...
test "iterator: resolves component columns separately for each archetype" {
    ...
    _ = world.addOwnedEntity(allocator, .{Position{ .x = 1, .y = 1 }});
    _ = world.addOwnedEntity(allocator, .{ Position{ .x = 2, .y = 2 }, Anchor{ .value = 7 } });
...
test "iterator: skips archetypes without the requested components" {
    ...
    _ = world.addOwnedEntity(allocator, .{Velocity{ .dx = 1, .dy = 1 }});
    _ = world.addOwnedEntity(allocator, .{Position{ .x = 5, .y = 5 }});
...
test "iterator: is independent of others made from the same query" {
    ...
    _ = world.addOwnedEntity(allocator, .{Position{ .x = 1, .y = 1 }});
    _ = world.addOwnedEntity(allocator, .{Position{ .x = 2, .y = 2 }});
...
test "iterator: filters on a marker without yielding storage" {
    ...
    _ = world.addOwnedEntity(allocator, .{Position{ .x = 3, .y = 4 }});
